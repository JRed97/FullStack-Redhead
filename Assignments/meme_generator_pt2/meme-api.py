...
@app.route('/user/<action>', methods=['POST','DELETE','OPTIONS','GET','PATCH','PUT'])
def user(action=None):
    ...
    if action == 'new':
        ...
        if request.method == "POST":     
            ...
            #get last id + 1 for new user
            count = client['memes_db']['users'].find().count()
            max_id = count + 1

            document['_id'] = max_id
            document['password'] = hashlib.sha224(document['password']).hexdigest()#hash password

            print(json.dumps(document, indent=4, sort_keys=True))

            test = client['memes_db']['users'].insert(document) #insert into db

            if type(test) is int:
                return jsonify({"success":True})
            else:
                return jsonify({"success":False,"error":"mongo insert error!"})
    ...
    if action == 'new':
        ...
        if request.method == 'POST':
            ...
            document = {
                ...
                "top_text" : request.form['topText'],
                "bot_text" : request.form['botText'],
                "style_info" : {
                    "text_color" : request.form['textColor'],
                    "text_size" : request.form['textSize'],
