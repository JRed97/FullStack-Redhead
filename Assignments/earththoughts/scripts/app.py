...
def get_quotes():
	quote_list = [] #list to hold quotes parsed from json file
	jsonquotes = json.loads(open('showerthoughts.json','r').read()) #open the showerthought file to read quotes
	for child in jsonquotes['data']['children']: #loop through file and append each quote to the list
		quote_list.append(child['data']['title'])
		
	random.shuffle(quote_list)
	return jsonify({"success":True,"url":quote_list[0]}) #returns the first quote from th shuffled list
